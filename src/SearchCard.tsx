...
const SearchUser: React.FC<SearchUserProps> = ({ username, setUsername, handleRequest, theme }) => {
    ...
    return (
        <div className='' >
            <section className={`w-full h-[4.5rem] flex flex-row items-center justify-between gap-4 pl-4 pr-3  ${theme === 'light' ? 'bg-[#fffffe]' : 'bg-[#1f2a48]'} rounded-[0.8rem]`}>
                <div className='flex flex-row items-center w-full gap-10'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="ml-6 w-7 h-7 stroke-[#017bfc]">
